-- ============================================================
-- Nkoroi Seed Fixup 2 — correct check constraints + FK targets
-- marks: recorded_by → profiles.id (not auth.users)
-- discipline_records: category values from check constraint enum
-- notices: category from enum; target_audience uses 'guardians' not 'parents'
-- ============================================================

-- ── marks (recorded_by FK → profiles.id) ──────────────────────────────────────
DO $$
DECLARE v_user uuid;
BEGIN
  SELECT id INTO v_user FROM public.profiles LIMIT 1;
  IF v_user IS NULL THEN
    RAISE NOTICE 'marks skipped: no profiles row found for recorded_by';
    RETURN;
  END IF;
...
-- ── discipline_records (valid category enum values from check constraint) ──────
DO $$
DECLARE v_user uuid;
BEGIN
  SELECT id INTO v_user FROM public.profiles LIMIT 1;
  IF v_user IS NULL THEN
    RAISE NOTICE 'discipline_records skipped: no profiles row for reported_by';
    RETURN;
  END IF;
...
-- ── notices (valid category + target_audience enums; guardians not parents) ────
DO $$
DECLARE v_user uuid;
BEGIN
  SELECT id INTO v_user FROM public.profiles LIMIT 1;
  IF v_user IS NULL THEN
    RAISE NOTICE 'notices skipped: no profiles row for posted_by';
    RETURN;
  END IF;
